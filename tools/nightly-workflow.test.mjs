...
test("given scheduled verification, when main is checked, then the full build records immutable first-attempt evidence", () => {
  ...
  assert.match(build, /nightly-release-evidence-/);
  assert.match(build, /needs: \[build, nightly-image\]/);
  assert.match(build, /contract: "coupled-nightly-image-v1"/);
  assert.match(build, /\$build == "success" and \$nightlyImage == "success"/);
