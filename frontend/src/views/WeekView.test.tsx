...
function freeSlot(courtNumber: number, slot: string) {
  return screen.getAllByTestId("free-slot").find((candidate) =>
    candidate.dataset.courtNumber === String(courtNumber) && candidate.dataset.slot === slot
  )!;
}

async function findFreeSlot(courtNumber: number, slot: string) {
  await screen.findAllByTestId("free-slot");
  return freeSlot(courtNumber, slot);
}
...
it("given the current week, when opening the next week, then its seven days are loaded", async () => {
  ...
  expect(screen.getByTestId("day-selector-2026-08-17")).toBeInTheDocument();
...
it("given another day is selected, when viewing it, then its occupancy table is shown", async () => {
  ...
  expect(screen.getByTestId("slot-heading-08:00")).toBeInTheDocument();
});
...
it("given eligibility refreshes overlap, when an older response arrives last, then the latest refusal remains", async () => {
  // given
  const older = deferred<Awaited<ReturnType<typeof api.bookingEligibility>>>();
  const latest = deferred<Awaited<ReturnType<typeof api.bookingEligibility>>>();
  vi.mocked(api.bookingEligibility)
    .mockResolvedValueOnce({ violations: [] })
  ...
  // when
  latest.resolve({ violations: [{ code: "booking.rule.noCourtBooking", params: {} }] });
  expect(await screen.findByTestId("booking-eligibility")).toBeInTheDocument();
  await act(async () => {
    older.resolve({ violations: [] });
    await older.promise;
  });

  // then
  await waitFor(() => expect(screen.getByTestId("booking-eligibility")).toBeInTheDocument());
  expect(freeSlot(1, "12:30").tagName).toBe("DIV");
});

it("given a booking dialog is open, when eligibility cannot be refreshed, then the dialog closes", async () => {
  // given
  vi.mocked(api.bookingEligibility)
    .mockResolvedValueOnce({ violations: [] })
  ...
  // then
  expect(await screen.findByTestId("booking-eligibility-error")).toBeInTheDocument();
  await waitFor(() => expect(screen.queryByRole("dialog")).not.toBeInTheDocument());
  expect(freeSlot(1, "12:30").tagName).toBe("DIV");
});
...
it("given today, when showing the plan, then current time and a return action are available", async () => {
  ...
  expect(screen.getByTestId("current-time")).toHaveRole("button");
});
...
it("given the browser and club have different dates, when loading, then the club date is selected", async () => {
  ...
  // then
  expect(await screen.findByTestId("day-selector-2026-08-11")).toHaveAttribute("aria-pressed", "true");
});
...
it("given a free slot, when booking it with a guest, then the refreshed allocation is visible", async () => {
  ...
  render(<WeekView today={clubInstant("07:00")} />);
  await findFreeSlot(1, "08:00");

  // when
  await userEvent.click(freeSlot(1, "08:00"));
...
it("given the booking dialog is open, when pressing escape, then focus returns to the selected slot", async () => {
  ...
  const slot = await findFreeSlot(1, "08:00");
  await userEvent.click(slot);
  expect((await screen.findByTestId("booking-dialog")).closest('[role="dialog"]')).toBeInTheDocument();
