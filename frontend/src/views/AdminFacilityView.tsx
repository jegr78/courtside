...
export function AdminFacilityView() {
  ...
  const [confirmedCourts, setConfirmedCourts] = useState<Record<string, AdminCourt>>({});
  const [confirmedHours, setConfirmedHours] = useState<Record<string, OpeningHours>>({});
  const [confirmedCards, setConfirmedCards] = useState<Record<string, BookingCard>>({});
  const [confirmedFillers, setConfirmedFillers] = useState<Record<string, ParticipantCard>>({});
  ...
  useEffect(() => {
    void Promise.all([api.adminCourts(), api.adminOpeningHours(), api.adminBookingCards(),
      ...
      .then(([loadedCourts, loadedHours, loadedCards, loadedFillers, configuration]) => {
        ...
        setConfirmedCourts(Object.fromEntries(loadedCourts.map((court) => [court.id, court])));
        setConfirmedHours(Object.fromEntries(loadedHours.map((day) => [String(day.dayOfWeek), day])));
        setConfirmedCards(Object.fromEntries(loadedCards.map((card) => [card.id, card])));
        setConfirmedFillers(Object.fromEntries(loadedFillers.map((card) => [card.id, card])));
      })
  ...
  function confirmCourt(changed: AdminCourt) {
    setCourts((current) => current?.some((item) => item.id === changed.id)
      ? current.map((item) => item.id === changed.id ? changed : item)
      : [...(current ?? []), changed]);
    setConfirmedCourts((current) => ({ ...current, [changed.id]: changed }));
  }

  function confirmHours(changed: OpeningHours) {
    replaceHours(changed);
    setConfirmedHours((current) => ({ ...current, [String(changed.dayOfWeek)]: changed }));
  }

  function confirmCard(changed: BookingCard) {
    setCards((current) => current?.some((item) => item.id === changed.id)
      ? current.map((item) => item.id === changed.id ? changed : item)
      : [...(current ?? []), changed]);
    setConfirmedCards((current) => ({ ...current, [changed.id]: changed }));
  }

  function confirmFiller(changed: ParticipantCard) {
    setFillers((current) => current?.some((item) => item.id === changed.id)
      ? current.map((item) => item.id === changed.id ? changed : item)
      : [...(current ?? []), changed]);
    setConfirmedFillers((current) => ({ ...current, [changed.id]: changed }));
  }
  ...
  async function closeDay(day: DayOfWeek) {
    ...
    try {
      ...
      confirmHours({ dayOfWeek: day, opensAt: null, closesAt: null });
      reportSuccess();
    } catch (failure) {
      reportError(failure);
    } finally {
    ...
    {!courts || !hours || !cards || !timeZone
      ...
      : <>
        ...
        <section className="grid gap-4">
          ...
          {courts.map((court) => <CourtEditor key={court.id} court={court} confirmed={confirmedCourts[court.id]} timeZone={timeZone} disabled={pending.has(`court:${court.id}`)} changed={(changed) => setCourts((current) => current?.map((item) => item.id === changed.id ? changed : item))} save={saveCourt} toggle={toggleCourt} reportError={reportError} />)}
          <form noValidate {...newCourt.form} onSubmit={(event) => void createCourt(event)} className="surface-subtle grid gap-3 rounded-xl border p-4 sm:grid-cols-[8rem_1fr_auto] sm:items-end">
        ...
        <section className="grid gap-4">
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            {hours.map((day) => <HoursEditor key={day.dayOfWeek} hours={day} confirmed={confirmedHours[String(day.dayOfWeek)]} timeZone={timeZone} disabled={pending.has(`hours:${day.dayOfWeek}`)} changed={replaceHours} save={saveHours} close={closeDay} reportError={reportError} />)}
          </div>
        </section>
        <section className="grid gap-4">
          <h2 className="text-2xl font-bold">{t("admin.facility.cards")}</h2>
          {cards.map((card) => <CardEditor key={card.id} card={card} confirmed={confirmedCards[card.id]} timeZone={timeZone} disabled={pending.has(`card:${card.id}`)} changed={(changed) => setCards((current) => current?.map((item) => item.id === changed.id ? changed : item))} save={saveCard} toggle={toggleCard} reportError={reportError} />)}
        ...
        <section className="grid gap-4">
          ...
          {(fillers ?? []).map((card) => <ParticipantCardEditor
            key={card.id}
            card={card}
            confirmed={confirmedFillers[card.id]}
