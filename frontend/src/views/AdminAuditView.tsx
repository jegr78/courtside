            ...
            <table className="block w-full text-left sm:table">
              ...
              <tbody className="grid gap-3 sm:table-row-group">
                {entries.map((entry) => <tr key={entry.id} data-testid="audit-row" data-entry-id={entry.id} data-subject-id={entry.subjectId} data-event-type={entry.eventType} className="grid gap-2 rounded-xl border p-4 sm:table-row sm:rounded-none sm:border-0 sm:p-0">
                  <td data-testid="audit-occurred-at" className="grid min-w-0 grid-cols-[minmax(0,8rem)_minmax(0,1fr)] gap-3 sm:table-cell sm:border-t sm:p-2">
                    <span data-testid="audit-label-occurred-at" className="font-medium sm:hidden">{t("audit.column.occurredAt")}</span>
                    <span className="min-w-0 break-words">{formatDateTime(entry.occurredAt, language, club.timeZone)}</span>
                  </td>
                  <td data-testid="audit-message" className="grid min-w-0 grid-cols-[minmax(0,8rem)_minmax(0,1fr)] gap-3 sm:table-cell sm:border-t sm:p-2">
                    <span data-testid="audit-label-change" className="font-medium sm:hidden">{t("audit.column.change")}</span>
                    <span className="min-w-0 [overflow-wrap:anywhere]">{auditMessage(entry, t)}</span>
                  </td>
                  <td data-testid="audit-subject" className="grid min-w-0 grid-cols-[minmax(0,8rem)_minmax(0,1fr)] gap-3 sm:table-cell sm:border-t sm:p-2">
                    <span data-testid="audit-label-subject" className="font-medium sm:hidden">{t("audit.column.subject")}</span>
                    <span className="min-w-0 [overflow-wrap:anywhere]">{subjectLabel(entry, t)}</span>
                  </td>
                  <td data-testid="audit-actor" className="grid min-w-0 grid-cols-[minmax(0,8rem)_minmax(0,1fr)] gap-3 sm:table-cell sm:border-t sm:p-2">
                    <span data-testid="audit-label-actor" className="font-medium sm:hidden">{t("audit.column.actor")}</span>
                    <span className="min-w-0 [overflow-wrap:anywhere]">{actorLabel(entry, t)}</span>
                  </td>
                </tr>)}
              </tbody>
            </table>
