...
| `COURTSIDE_IMPORT_PREVIEW_RETENTION` | `7d` | How long a roster-import preview keeps the change set it resolved. The uploaded file itself is never kept — only its SHA-256. Past this bound the row, its hash and its counts survive and the change set does not. |
| `COURTSIDE_IMPORT_SWEEP_INTERVAL` | `1h` | How often previews past their retention are swept. The sweep drops the resolved change set and keeps the row, its hash and its counts. |
