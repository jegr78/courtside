...
| `COURTSIDE_SESSION_INACTIVITY_TIMEOUT` | `30m` | How long a sign-in survives without a request. This is the window a member notices: it restarts with every request, so an active session is never interrupted by it. At least one minute; below that it is refused at startup, because Spring Session reads a negative interval as one that never expires and such a value would switch the window off rather than tighten it. Stated here rather than inherited, because an unset value is whatever the framework defaults to and that can change under an installation. |
