...
| `COURTSIDE_SESSION_MAX_CONCURRENT` | `5` | How many sign-ins one account may hold at the same time — a phone, a tablet and a club laptop are three of them. A further sign-in is not refused: it succeeds and the account's least recently active session is ended instead, so nobody is locked out by a device they cannot reach. Between 1 and 50; outside that it is refused at startup, because a limit no account reaches is the policy switched off while the variable still reads as if it were set. |
