...
| `COURTSIDE_SESSION_ABSOLUTE_LIFETIME` | `24h` | How long a sign-in may live at all, counted from when it began and not restarted by activity. A session past it is ended on its next request and the member signs in again, however busy the session was — the sign-in itself is never what gets refused. The count survives a restart, because it is stored with the session rather than held in memory. It runs from when the browser first got a session, which for a member who had the page open before signing in is a little earlier than the sign-in. It may not be shorter than `COURTSIDE_SESSION_INACTIVITY_TIMEOUT`; that combination is refused at startup, because the inactivity window could then never be reached and setting it would say nothing. |
