...
| `COURTSIDE_ENVIRONMENT` | `PRODUCTION` | Public environment designation: `PRODUCTION`, `UAT`, `DEVELOPMENT`, `PERFORMANCE` or `SECURITY`. UAT is visibly marked in the frontend. `SECURITY` belongs to the disposable assessment target and additionally answers every request with the host and scheme the application observed; a club has no reason to set it. |
