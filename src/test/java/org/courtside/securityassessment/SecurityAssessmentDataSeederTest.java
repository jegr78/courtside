...
class SecurityAssessmentDataSeederTest {
    ...
    void givenFreshDatabase_whenSeedingSecurityData_thenEveryRoleHasTwoAccountsAndBoundaryDataExists() {
        ...
        SecurityAssessmentDataSeeder seeder = new SecurityAssessmentDataSeeder(
                ...
                new SecurityAssessmentProperties(true, "run-0001", SecurityAssessmentDataSeeder.SEED_FINGERPRINT,
        ...
        verify(accounts, times(SecurityAssessmentDataSeeder.ROLE_ACCOUNT_COUNT + 1)).save(account.capture());
        ...
        verify(members, times(SecurityAssessmentDataSeeder.ROLE_ACCOUNT_COUNT + 1)).save(member.capture());
        assertThat(member.getAllValues()).anySatisfy(value -> assertThat(value.isCurrent()).isFalse());
        verify(bookings, times(2)).save(any());
        verify(jdbc, times(2)).update(any(String.class), any(Object[].class));
