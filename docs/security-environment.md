...
To replay an OpenAPI candidate, keep its protected `openapi-fuzz.json`, start a fresh environment with the recorded application image, and run the recorded active profile against the unchanged policy and OpenAPI digests. Locate the new counterexample by operation, mode, check and minimized request, then compare its structural `reason`. The reproduction digest matches only when the seed, operation, mode, check, reason and minimized request all match. A different generated identifier may change the request digest, but the same schema pointer, validation keyword, status class or media-type disagreement is sufficient to validate the structural defect. Never copy a discarded raw Schemathesis report into retained evidence.
