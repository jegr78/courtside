...
The safe deployment suite runs native TLS, HTTP, proxy and container checks plus the pinned ZAP passive baseline against `SECURITY`. Its native checks include representative sensitive and backup paths, unsafe methods and method-override headers, protocol and cipher negotiation, and the application image's runtime file permissions. ZAP can reach only the internal proxy network. A scanner alert is an untriaged candidate, and a run holding one is incomplete, until a record in [`passive-alert-dispositions.json`](../security/passive-alert-dispositions.json) resolves it. The record carries the reason, who classified it, when and against which source, and either dismisses the alert itself or names an acceptance in [`exceptions.json`](../security/exceptions.json) that must exist, carry this alert's fingerprint and not have expired. A record covers an alert only when the rule, method, route template, risk, confidence, scanner version and rule-specific observation all match what it names, so a louder or different alert on the same route is a new one. The observation is what the rule concluded rather than where it looked: the set of comment patterns, the directive names, the token names. A comment rule's resource digest changes with every frontend build, so binding to it would expire every record on a rebuild while telling nobody anything new. The observation carries every wording the rule used, because the scanner merges them: one rule on one route arrives as a single alert, and a policy wording that names no directive would otherwise leave the directive union unmoved and go unnoticed. Each of rule 10055's ten templates is named, and a wording none of them explains gets a stable name of its own. How often the rule fired is provenance rather than identity: a route template stands for however many URLs the crawl reached, so binding the count would expire a triage decision the next time the crawl found one more asset. The run records the day it read those records, an acceptance stops resolving its alert once that day is past its expiry, and that day cannot precede the records it relied on. An alert no record covers keeps the run incomplete, which is how a new one is noticed. The build recomputes every state as it writes the evidence, which catches a record that has drifted from what the run observed; it is not a check on a published artefact, which is only as trustworthy as the run that produced it. The active suite adds the OpenAPI authorization matrix, authenticated ZAP scans and the pinned Schemathesis boundary suite. The destructive suite adds only the bounded resource-abuse adapter described below. Product vulnerabilities found by a suite are fixed separately so framework changes do not conceal product changes.
...
Browser security reuses that packaged PWA suite rather than starting a second browser environment. Chromium projects inert stored markup through the maintained rendering-context inventory, records normalized CSP violations, and inventories console event types, Web Storage keys and cache request paths without retaining their values. It repeats the storage, cache and cookie inventory after logout and browser-history navigation. Security journeys retain no raw trace or screenshot. WebKit and periodic Firefox runs verify the CSP and clickjacking headers. The service-worker lifecycle journey admits only the current member's `/api/my/bookings` response while authenticated, proves its bounded offline use, and requires login, logout and a server-observed session expiry to delete it; the worker-update path rechecks CSP and the closed API-cache allowlist.

Authenticated ZAP uses a separate synthetic session for each role. The scanner receives the cookie only through a mode-`0600` file in its container tmpfs. It spiders read-only routes for all roles and runs the two curated query-only active rules for `MEMBER` and `ADMIN`. The gateway rejects non-canonical paths, other methods and foreign origins before the request reaches Caddy. Every role proves its session again after its scan jobs. A scanner-only response exposes a harmless header that the pinned passive rule must detect. The runner then recreates the gateway without that header and uses a separate, closed ZAP plan to prove that the same rule no longer reports the exact canary path. The retained lifecycle proof binds validation, remediation, fix and successful retest to their event times, the retest request count and its protected report digest. Product candidates remain separate and keep the attempt incomplete until triage resolves them. A missing canary, failed retest, lost session, missing role coverage or unexpected rule also makes the attempt incomplete. Raw ZAP reports and cookies are discarded with the container. Retained evidence binds the policy and all executed plans by digest and contains normalized candidate fingerprints but no URLs, payloads, response bodies or authentication material.
