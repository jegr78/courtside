...
Browser security reuses that packaged PWA suite rather than starting a second browser environment. Chromium projects inert stored markup through every current text-bearing UI surface, records CSP violations, and inventories console event types, Web Storage keys and cache request paths without retaining their values. WebKit and periodic Firefox runs verify the CSP and clickjacking headers. The service-worker update journey rechecks both CSP and the absence of API responses from Cache Storage after activation.
