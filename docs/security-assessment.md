...
limits, request-body rejection, competing court occupancy, participant-card capacity, maximum
series previews and preview-to-mutation races. It has no host network, Docker socket or direct
application network.
...
samples beyond a declared limit stop attack traffic and make the result incomplete. Passing requires
exactly one surviving competing booking, no partial operation, restoration of the domain-state
fingerprint, a healthy database and a successful application restart. Capacity latency and
throughput remain performance evidence under `performance/`; safety-limit breaches belong to the
security run.
