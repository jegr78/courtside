...
A manual procedure covers a chapter, so the procedure a control links to says nothing specific about that control. A control that has been reviewed against one named production path additionally carries `controlEvidence`: the production path it was read against, and one test, written as `<path>#<name>`, that goes red when the behavior stops. The contract test resolves both, so an anchor that names a path or a test the repository does not contain fails the build.

That anchor is what a pass costs. `validateManualAssessmentEvidence` refuses `pass` for a control that carries neither `controlEvidence` nor `assessmentTestIds`, so an outcome derived from a chapter-level template alone cannot be recorded. A control without an anchor can still be recorded `fail`, `not-applicable` or `blocked`; the first complete manual run recorded 146 of them as blocked for exactly that reason, and [`manual-baseline-control-outcomes.json`](../security/manual-baseline-control-outcomes.json) names which.
