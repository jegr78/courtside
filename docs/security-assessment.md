...
Schemathesis inventories every OpenAPI operation under a fixed seed. Read-only operations receive valid and invalid generated requests. Mutations receive invalid requests only; valid mutations remain in the packaged journeys and authorization suite so the state comparison has one meaning. Authentication transitions and import execution have explicit exclusions. Four curated multipart cases cover invalid UTF-8, duplicate columns, an oversized cell and a conflicting external reference. The runner fingerprints roster, booking and configuration state immediately before and after the rejected-input phase. It excludes session counters and immutable preview records from that comparison.

The scanner runs with one worker and an in-memory generation database. Its image, policy and OpenAPI bytes are digest-bound. Raw request and response data stays in container tmpfs and is deleted with the container. Retained evidence contains operation IDs, path templates, modes, the seed and hashes of minimized cases. An unexpected observed operation becomes a finding candidate; a missing operation, changed state or incomplete import result prevents a pass. Domain property tests remain separate because they exercise Java value spaces without HTTP, proxy, authentication or serialization behavior.
