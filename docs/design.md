...
Flyway applies migrations at startup. Until the first stable release is published, a correction may
...
The public recipes route traffic through Caddy, which terminates TLS and replaces forwarding
headers. The `funnel` and `existing-infrastructure` recipes publish the application on a loopback
port for an operator-owned HTTPS ingress. In those recipes, that ingress owns forwarded-header
normalization, request limits and the response policies otherwise added by Caddy.
...
certificate. Other recipes use an operator-selected relay.
