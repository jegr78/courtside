...
| P2 | Material degradation with a safe workaround and no durable integrity or confidentiality loss | May ship only with an owner, rationale and workaround. |
| P3 | Limited defect with low user impact and no effect on a protected invariant | May ship as tracked follow-up work. |

Likelihood never lowers security breaches, privilege escalation, double occupancy, data loss or unrecoverable releases below P0. An exception records the affected risk IDs, owner, compensating controls and evidence. In the current single-maintainer project, one maintainer may approve an exception; the absence of independent review is recorded.

No row carries a review date. A periodic re-reading of the register is the work of whoever operates
an instance, and this repository ships an image rather than running one. A risk row changes when the
code it describes changes, and a vulnerability is reported through private vulnerability reporting
rather than waiting for a calendar to raise it.
...
- A risk accepted without automation appears in the gap register with an owner and a decision.
...
- [ ] Review open P2/P3 exceptions and automation gaps for ownership and a current decision.
...
| ID | Impact | Likelihood | Invariant | Positive boundaries | Negative boundaries | Level | Frequency | Environment | Synthetic data | Evidence | Owner | Open gap |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
| UI-2 | P0 | Low | PWA caching and browser history serve only the current member's bounded booking cache offline and never retain it after logout, a server-observed expiry or account change. | Install, update, offline launch and reconnect keep personal bookings readable with an honest refresh time while the live court plan stays unavailable. | Logout, a server-observed expiry, account change, Back or Forward, old assets and multiple tabs cannot reveal a previous member's cached data. | React, E2E, deployment smoke | PR, nightly, release | Chromium, WebKit and periodic Firefox | Synthetic member sessions | Browser traces, cache inspection and linked device record | Frontend and identity | Physical-device evidence remains manual because emulation cannot prove operating-system integration. |
...
| ID | Risks | Gap | Owner | Decision | Issue |
|---|---|---|---|---|---|
| GAP-COURT-DEACTIVATION | AD-1 | The product permits an administrator to deactivate every court. | Configuration and facility | The existing issue remains authoritative; do not duplicate it. | [#41](https://github.com/jegr78/courtside/issues/41) |
<!-- automation-gap-register:end -->

New material gaps are added here or linked to an existing authoritative issue before the change is considered qualified. A gap closes when the automation it names exists, not when a date passes.
...
scheduled build, and it reads no calendar: schema, uniqueness and referential integrity hold on any
day, so the check answers the same way whenever it runs. A release review
