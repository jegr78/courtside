...
Every risk records its last review and next due date. P0 reviews are due within 90 days, P1 within
180 days, and P2 or P3 within 365 days. A linked acceptance or protected evidence expiry moves the
next review earlier.
...
The remote npm advisory audit runs every Tuesday, on manual request and during a release. Pull requests use Dependency Review for changed dependencies instead of waiting for that remote service. A release records an npm-reported network failure or HTTP 502, 503 or 504 response as `skipped` and continues; an audit that completed with High or Critical findings still blocks publication. A locally expired process budget remains a failure because it carries no evidence about the service.
...
Every Maven verification produces JaCoCo line and branch reports for hand-written backend code and V8 coverage reports for frontend source. Generated OpenAPI models, generated TypeScript declarations, test support and framework output are excluded because their execution does not demonstrate a Courtside decision. The 2026-09-05 baseline on commit `263d9ff3275dc072669c9978f7a62ede35ae8b59` measured 96.18% line and 83.52% branch coverage with JaCoCo 0.8.15, and 89.79% statement, 85.94% branch, 86.79% function and 92.03% line coverage with Vitest 4.1.11. It ran on Darwin arm64 with Temurin 25.0.2 and Node.js 26.8.1. Verification holds the floors at 96% and 83% for JaCoCo, and 89%, 85%, 86% and 92% for the corresponding Vitest counters. These floors prevent an unnoticed decline; additional assertions remain justified by a risk boundary rather than by chasing a percentage.
...
| ID | Impact | Likelihood | Invariant | Positive boundaries | Negative boundaries | Level | Frequency | Environment | Synthetic data | Evidence | Owner | Open gap | Last review | Next review |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| OP-1 | P0 | Medium | The exact released image starts securely behind the reference proxy and preserves data across restart. | The exact image starts on an empty database and preserves data across a controlled restart. | Read-only filesystem, non-root execution, TLS, cookie and management-isolation checks cannot be bypassed. | Deployment smoke, release qualification | release | Reference Compose topology | Production-shaped synthetic club | Archive- and image-digest qualification report on both architectures | Operations and release | The public network and host remain operator-owned and are reported separately from software results. | 2026-09-17 | 2026-12-16 |
| OP-2 | P0 | Medium | Every supported upgrade preserves data and either completes atomically or fails without serving inconsistent state. | Previous patch, minor and supported skipped-version upgrades preserve data and invariants. | Interrupted or invalid migrations fail without serving an inconsistent schema. | CLI contract, release qualification | PR, release | PostgreSQL 17 and candidate image | Version-owned synthetic dataset | CLI state-transition tests, migration logs and invariant checksums | Database and release | No downgrade is promised after a newer schema has run; recovery uses the pre-update recovery unit. | 2026-09-17 | 2026-12-16 |
| OP-3 | P0 | Low | A documented backup restores into an empty compatible environment and passes functional and integrity checks. | The application writes a synthetic club through its public API; its backup and the legacy-schema fixture each restore into empty PostgreSQL 17, and the matching image reads the restored login, booking and logo. | A corrupt archive leaves no database objects, an interrupted restore preserves the usable database, and the restored exclusion constraint still rejects overlap. | CLI contract, release qualification | PR, periodic, release | Fresh PostgreSQL 17 | API-written and legacy-schema synthetic datasets, including an outstanding event publication | CLI recovery-unit checks, before-and-after table and sequence evidence, backup sizes, image ID and schema version | Operations | The relay is stopped before a second API booking so the backup contains an application-written outstanding Modulith publication; remote storage remains operator-owned. | 2026-09-17 | 2026-12-16 |
| OP-4 | P1 | Medium | Required gates are reproducible, isolated, diagnosable and cannot turn a missing tool or retry into success. | Independent fixtures pass under normal and varied order with retained first-attempt evidence. | Concurrency timeout, tool outage, cleanup failure and rerun cannot be reported as a clean first attempt. | All levels | PR, nightly | CI and reference runners | Independent deterministic fixtures | First-attempt result and privacy-safe diagnostics | Build and quality | Periodic backend order and configured/reversed browser-project runs measure isolation without retries. | 2026-09-05 | 2027-03-04 |
<!-- risk-register:operations-and-release:end -->

## Automation gap register

This table is generated from the same register. Every gap names at least one maintained risk.

<!-- automation-gap-register:start -->
| ID | Risks | Gap | Owner | Review date | Decision | Issue |
|---|---|---|---|---|---|---|
| GAP-COURT-DEACTIVATION | AD-1 | The product permits an administrator to deactivate every court. | Configuration and facility | 2027-03-04 | The existing issue remains authoritative; do not duplicate it. | [#41](https://github.com/jegr78/courtside/issues/41) |
<!-- automation-gap-register:end -->
...
Pull requests update the machine-readable register when they add a product surface, alter an
invariant, change supported environments, introduce an accepted risk or move evidence between
gates. The documentation check renders and validates this contract on pull requests and on the
scheduled build. The existing nightly failure tracker opens or updates an issue when that scheduled
check finds an overdue review, expired acceptance, expired evidence or orphaned ID. A release review
checks every row whose frequency includes `release`. The document stays at risk level: individual
classes, methods and scanner rules remain discoverable from executable configuration and must not
be copied here.
